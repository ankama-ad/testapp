...
async function getEmbedParamsForSingleReport(workspaceId, reportId, additionalDatasetId) {
    const reportInGroupApi = `https://api.powerbi.com/v1.0/myorg/groups/${workspaceId}/reports/${reportId}?filter=Sales lt 3000000`;
    const headers = await getRequestHeader();
    const reportexportUrl = `https://api.powerbi.com/v1.0/myorg/reports/${reportId}/Export`
    ...
    // const zip =  await fetch(reportexportUrl, {
    //     method: 'POST',
    //     headers: headers,
    //     body: {format: 'pdf', size: ''}
    // })
    // if(zip){
    //     return zip;
    // }
